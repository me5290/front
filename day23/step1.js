...
document.addEventListener('DOMContentLoaded' , function(){
    ...
    textarea.addEventListener('keyup' , function(e){
        console.log(e);             // *event : 매개변수로 이벤트의 정보('keyup')를 받습니다.
    ...
    // 3. 별 위치를 이동하는 이벤트
        ...
        const right = 39;
        const down = 40;
        // 3-2 body에서 키를 눌렀을때 이벤트 발생
        document.body.addEventListener('keydown' , function(e){
            console.log(e);
            if(e.keyCode == left){          // 왼쪽방향키를 눌렀을때 x 1감소
                x--;
            }else if(e.keyCode == up){      // 위쪽방향키를 눌렀을때 y 1감소
                y--;
            }else if(e.keyCode == right){   // 오른쪽방향키를 눌렀을때 x 1증가
                x++;
            }else if(e.keyCode == down){    // 아래쪽방향키를 눌렀을때 y 1증가
                y++;
            }else{
